...
def svm_loss_naive(
    ...
    num_train = X.shape[0]
    loss = 0.0
    for i in range(num_train): # for each sample
        scores = W.t().mv(X[i]) # length c, num_dim=1
...
def svm_loss_vectorized(
    ...
    data_dim = X.shape[1] # D
    feat = torch.matmul(W.t(),X.t()) # every column is the feature of a sample, [C,N]
    row_id = y
    col_id = torch.arange(0,num_train,1)
    correct_class_feat = feat[row_id,col_id]
    correct_class_feat = correct_class_feat.view(1,-1).repeat(num_classes,1)
    
    margin = feat - correct_class_feat + 1 # [C,N]
    margin[margin < 0] = 0
    margin[row_id,col_id] = 0 # skip "j == y[i]" condition
    ...
    # calculate dW[:,j] += X[i] without loops
    margin[margin > 0] = 1
    margin[row_id,col_id] = -torch.sum(margin, dim=0)
    dW = margin.mm(X).t()
...
def sample_batch(
    ...
    #########################################################################
    # Replace "pass" statement with your code
    sample_id = torch.randint(0,num_train,(batch_size,))
    X_batch = X[sample_id]
    y_batch = y[sample_id]
    #########################################################################
    #                       END OF YOUR CODE                                #
...
def svm_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    # learning_rates = [1*(0.1**i) for i in range(5)]
    # regularization_strengths = [1e-10*(10**i) for i in range(5)]
...
def test_one_param_set(
    ...
    train_acc = torch.mean((train_y_pred==data_dict['y_train']).float())
    train_acc = train_acc.cpu()
    val_y_pred = cls.predict(data_dict['X_val'])
    val_acc = torch.mean((val_y_pred==data_dict['y_val']).float())
    val_acc = val_acc.cpu()
    ############################################################################
    #                            END OF YOUR CODE                              #
    ############################################################################
...
################ Section 2: Softmax ################
# **************************************************#
# softmax loss is actually cross-entropy loss with softmax
def softmax_fun(p: torch.Tensor, dim: int):
    return torch.exp(p)/torch.exp(p).sum(dim=dim) # let softmax apply on the last dimension
...
def softmax_loss_naive(
    ...
    for i in range(num_train):
        scores = W.t().mv(X[i])
        scores -= scores.amax(dim=0)
        softmax_scores = softmax_fun(scores,dim=0)
        loss -= torch.log(softmax_scores[y[i]])
        
        label_scores = torch.zeros_like(scores)
        label_scores[y[i]] = 1.0
        d_scores = softmax_scores - label_scores
        dW += d_scores.view(1,-1) * X[i].view(-1,1)
...
def softmax_loss_vectorized(
    ...
    label_scores = torch.zeros_like(scores)
    label_scores[torch.arange(num_train),y] = 1.0
    d_scores =  softmax_scores - label_scores
    dW += X.t() @ d_scores
    dW /= num_train
    dW += reg * 2 * W
