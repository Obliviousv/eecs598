...
def nn_forward_backward(
    ...
    loss = -torch.log(softmax_scores[torch.arange(N),y]).sum()
    loss /= N
    ...
    label_scores[torch.arange(N),y] = 1.0
    d_scores =  softmax_scores - label_scores
    grads['W2'] = h1.t() @ d_scores / N + reg * 2 * W2
    grads['b2'] = d_scores.sum(dim=0)/N
    # W1, b1 grads
    d_h1 = d_scores @ W2.t() # after relu
    d_h1[h1==0] = 0 # before relu
    grads['W1'] = X.t() @ d_h1 / N + reg * 2 * W1
    grads['b1'] = d_h1.sum(dim=0)/N
...
def nn_train(
    ...
    for it in range(num_iters):
        ...
        #########################################################################
        # Replace "pass" statement with your code
        for para_n, para_v in grads.items():
            params[para_n] -= grads[para_n] * learning_rate
        #########################################################################
        #                             END OF YOUR CODE                          #
        #########################################################################
...
def find_best_net(
    ...
    #############################################################################
    # Replace "pass" statement with your code
    (learning_rates, hidden_sizes, reg_strengths, learning_rate_decays) = get_param_set_fn()
    cnt = 0
            ...
            for reg in reg_strengths:
                for lr_decay in learning_rate_decays:
                    ...
                    if val_acc > best_val_acc:
                        ...
                        best_hyper_paras = [lr, hs, reg, lr_decay]
                    cnt += 1
                    print(cnt)
    #############################################################################
    #                               END OF YOUR CODE                            #
    ...
    return best_net, best_stat, best_val_acc, best_hyper_paras
