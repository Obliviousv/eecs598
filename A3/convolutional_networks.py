...
class Conv(object):
    ...
    def forward(x, w, b, conv_param):
        ...
        x_pad = torch.zeros((N,C,H+2*pad,W+2*pad), dtype=dtype, device=device)
        x_pad[:,:,pad:H+pad,pad:W+pad] = x
        x_pad_r = x_pad.reshape((N,1,C,H+2*pad,W+2*pad))
        w_r = w.reshape(1,F,C,HH,WW)
        b_r = b.reshape(1,F)

        H_ = 1 + (H + 2 * pad - HH) // stride
        W_ = 1 + (W + 2 * pad - WW) // stride
        out = torch.zeros((N,F,H_,W_), dtype=dtype, device=device)

        for i in range(H_):
          for j in range(W_):
              x_filter = x_pad_r[:,:,:,stride*i:stride*i+HH,stride*j:stride*j+WW]
              out[:,:,i,j] = (x_filter*w_r).sum(dim=(-1,-2,-3))+b_r
    ...
    def backward(dout, cache):
        ...
        x_pad = torch.zeros((N,C,H+2*pad,W+2*pad), dtype=dtype, device=device)
        x_pad[:,:,pad:H+pad,pad:W+pad] = x
        x_pad = x_pad.reshape((N,1,C,H+2*pad,W+2*pad))

        w_r = w.reshape(1,F,C,HH,WW)
        dout_r = dout.reshape(N,F,1,H_,W_)

        dx = torch.zeros_like(x)
        dx_pad = torch.zeros((N,C,H+2*pad,W+2*pad), dtype=dtype, device=device)
        for i in range(H_):
            for j in range(W_):
                dout_r_partial = dout_r[:,:,:,i,j].reshape(N,F,1,1,1)
                delta_dx = (dout_r_partial*w_r).sum(dim=1)
                a,b=i*stride,j*stride
                dx_pad[:,:,a:a+HH,b:b+WW] += delta_dx
        dx = dx_pad[:,:,pad:H+pad,pad:W+pad]
        
        dw=torch.zeros_like(w)
        for i in range(HH):
            for j in range(WW):
                x_pad_mask = x_pad[:,:,:,i:i+H_*stride:stride,j:j+W_*stride:stride]
                dw[:,:,i,j] = (dout_r*x_pad_mask).sum(dim=(-1,-2,0))
        # db=torch.zeros_like(b)
        db=dout.sum(dim=(-1,-2,0))
        ###############################################################
        #                       END OF YOUR CODE                      #
        ###############################################################
        return dx, dw, db
...
class MaxPool(object):
    ...
    def forward(x, pool_param):
        ...
        out = torch.zeros((N,C,H_,W_), dtype=dtype, device=device)
        for i in range(H_):
          for j in range(W_):
              h_st,w_st = i*stride,j*stride
              x_area = x[:,:,h_st:h_st+pool_height,w_st:w_st+pool_width]
              out[:,:,i,j] = torch.amax(torch.amax(x_area,dim=-1),dim=-1)
        ####################################################################
        #                         END OF YOUR CODE                         #
        ####################################################################
        cache = (x, pool_param)
    ...
    def backward(dout, cache):
        ...
        for i in range(H_):
          for j in range(W_):
            dout_f = dout[:,:,i,j].flatten()

            h_st, w_st = i*stride,j*stride
            x_area = x[:,:,h_st:h_st+pool_height,w_st:w_st+pool_width]
      
            x_area_row = torch.amax(x_area, dim=-1)
            h_id = h_st + torch.argmax(x_area_row, dim=-1).flatten()
          
            x_area_col = torch.amax(x_area, dim=-2)
            w_id = w_st + torch.argmax(x_area_col, dim=-1).flatten()
      
            row_id = [n for n in range(N) for c in range(C)]
            col_id = [c for n in range(N) for c in range(C)]

            dx[row_id,col_id,h_id,w_id] += dout_f
...
class ThreeLayerConvNet(object):
    ...
    def __init__(self,
        ...
        H_ //= 2
        W_ //= 2
        self.act_shape['out_conv_pool'] = (F,H_,W_)
        ...
        # linear layer 1
        self.params['W2'] = torch.randn((F*H_*W_,hidden_dim), dtype=dtype, device=device)*weight_scale
    ...
    def loss(self, X, y=None):
        ...
        out_conv = out_conv.reshape((N,-1))

        out_fn1, cache_fn1 = Linear_ReLU.forward(out_conv, W2, b2)
        out_fn2, cache_fn2 = Linear.forward(out_fn1, W3, b3)
        scores = out_fn2
        ######################################################################
        #                             END OF YOUR CODE                       #
        ######################################################################
        ...
        d_out_fn1, d_W3, d_b3 = Linear.backward(d_scores,cache_fn2)
        d_out_conv, d_W2, d_b2 = Linear_ReLU.backward(d_out_fn1,cache_fn1)
        
        F,H_,W_ = self.act_shape['out_conv_pool']
        d_out_conv = d_out_conv.reshape(N,F,H_,W_)

        _, d_W1, d_b1 = Conv_ReLU_Pool.backward(d_out_conv,cache_conv)

        grads['W3'] = d_W3 + 2*self.reg*W3
...
class DeepConvNet(object):
    ...
    def __init__(self,
        ...
        for i in range(1,self.num_layers):
            ...
            if weight_scale!='kaiming':
              self.params['conv_w'+str(i)] = torch.randn((F,F_past,ks,ks), dtype=dtype, device=device)*weight_scale
            else:
              self.params['conv_w'+str(i)] = kaiming_initializer(F_past,F,ks,True,device,dtype) # every conv operation include D_in*ks*ks weight params
            self.params['conv_b'+str(i)] = torch.zeros((F), dtype=dtype, device=device)
            if self.batchnorm:
              self.params['bn_gamma'+str(i)] = 0.1*torch.randn((F), dtype=dtype, device=device)
              self.params['bn_beta'+str(i)] = 0.1*torch.randn((F), dtype=dtype, device=device)
            F_past = F
        
        num_max_pool = len(self.max_pools)
        H_linear = H//(2**num_max_pool)
        W_linear = W//(2**num_max_pool)
        if weight_scale!='kaiming':
          self.params['linear_w'+str(self.num_layers)] = torch.randn((F*H_linear*W_linear,num_classes), dtype=dtype, device=device)*weight_scale
        else:
          self.params['linear_w'+str(self.num_layers)] = kaiming_initializer(F*H_linear*W_linear,num_classes,None,False,device,dtype)
        self.params['linear_b'+str(self.num_layers)] = torch.zeros((num_classes), dtype=dtype, device=device)
    ...
    def loss(self, X, y=None):
        ...
        cache_conv_list = []
        for i in range(1,self.num_layers):
            conv_w = self.params['conv_w'+str(i)]
            conv_b = self.params['conv_b'+str(i)]
            if self.batchnorm:
              bn_param = self.bn_params[i-1]
              bn_gamma = self.params['bn_gamma'+str(i)]
              bn_beta = self.params['bn_beta'+str(i)]
              if (i-1) in self.max_pools:
                out_conv, cache_conv = Conv_BatchNorm_ReLU_Pool.forward(out_conv, conv_w, conv_b, bn_gamma, bn_beta, conv_param, bn_param, pool_param)
              else:
                out_conv, cache_conv = Conv_BatchNorm_ReLU.forward(out_conv, conv_w, conv_b, bn_gamma, bn_beta, conv_param, bn_param)
            else:
              if (i-1) in self.max_pools:
                out_conv, cache_conv = Conv_ReLU_Pool.forward(out_conv, conv_w, conv_b, conv_param, pool_param)
              else:
                out_conv, cache_conv = Conv_ReLU.forward(out_conv, conv_w, conv_b, conv_param)
            
            cache_conv_list.append(cache_conv)

        N,C_linear,H_linear,W_linear = out_conv.shape
        out_conv = out_conv.reshape(N,-1)
        linear_w = self.params['linear_w'+str(self.num_layers)]
        linear_b = self.params['linear_b'+str(self.num_layers)]
        out_linear, cache_linear = Linear.forward(out_conv, linear_w, linear_b)
        scores = out_linear
        #####################################################
        #                 END OF YOUR CODE                  #
        #####################################################
        ...
        for i in range(1,self.num_layers):
          conv_w = self.params['conv_w'+str(i)]
          loss += self.reg*(torch.sum(conv_w*conv_w))
        linear_w = self.params['linear_w'+str(self.num_layers)]
        loss += self.reg*(torch.sum(linear_w*linear_w))

        dout_linear, dW_linear, db_linear = Linear.backward(d_scores,cache_linear)
        grads['linear_w'+str(self.num_layers)] = dW_linear + 2*self.reg*self.params['linear_w'+str(self.num_layers)]
        grads['linear_b'+str(self.num_layers)] = db_linear
        dout_conv = dout_linear
        dout_conv = dout_conv.reshape(N,C_linear,H_linear,W_linear)

        for i in range(self.num_layers-1,0,-1):
            cache_conv = cache_conv_list[i-1]
            if self.batchnorm:
              if (i-1) in self.max_pools:
                dout_conv, dW_conv, db_conv, dgamma, dbeta = Conv_BatchNorm_ReLU_Pool.backward(dout_conv,cache_conv)
              else:
                dout_conv, dW_conv, db_conv, dgamma, dbeta = Conv_BatchNorm_ReLU.backward(dout_conv,cache_conv)
            else:
              if (i-1) in self.max_pools:
                dout_conv, dW_conv, db_conv = Conv_ReLU_Pool.backward(dout_conv,cache_conv)
              else:
                dout_conv, dW_conv, db_conv = Conv_ReLU.backward(dout_conv,cache_conv)
            grads['conv_w'+str(i)] = dW_conv + 2*self.reg*self.params['conv_w'+str(i)]
            grads['conv_b'+str(i)] = db_conv
            if self.batchnorm: # Do not apply normalization to bn layer's params
...
class BatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        # return torch.zeros if 'running_mean' does not exist in the keys of bn_param
        ...
        if mode == 'train':
            ...
            batch_var = torch.sum((x-batch_mean)**2, dim=0)/N
            running_var = momentum*running_var + (1-momentum)*batch_var
            
            x_ = (x-batch_mean)/(batch_var+eps)**0.5
            out = gamma*x_ + beta
            cache = (batch_mean, batch_var, eps, x_, x, gamma)
            ################################################################
            #                           END OF YOUR CODE                   #
            ################################################################
        elif mode == 'test':
    ...
    def backward(dout, cache):
        ...
        batch_mean, batch_var, eps, x_, x, gamma = cache
        N = x.shape[0]
        b_e = (batch_var+eps)
        x_u = x-batch_mean 
        
        dx_ = dout*gamma 
        dvar = (dx_*x_u*-0.5*b_e**-1.5).sum(dim=0)
        dmean = (-dx_*b_e**-0.5).sum(dim=0) - 2*dvar*x_u.sum(dim=0)/N
        
        dx = dx_*b_e**-0.5 + dvar*2*x_u/N + dmean/N
    ...
    def backward_alt(dout, cache):
        ...
        ###################################################################
        # Replace "pass" statement with your code
        u, v, eps, x_, x, gamma = cache
        N = x.shape[0]
        s = (v+eps)**-0.5
        x_mu = x-u
        dgamma = (dout*x_).sum(dim=0)
        dbeta = dout.sum(dim=0)
        dx = gamma*s*(dout-dbeta/N-x_mu*dgamma*s/N)
        #################################################################
        #                        END OF YOUR CODE                       #
        #################################################################
...
class SpatialBatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        # Replace "pass" statement with your code
        N,C,H,W = x.shape
        x_reshape = x.permute(1,0,2,3).reshape(C,-1).t()
        out_reshape,cache = BatchNorm.forward(x_reshape, gamma, beta, bn_param)
        out = out_reshape.t().reshape(C,N,H,W).permute(1,0,2,3)
    ...
    def backward(dout, cache):
        ...
        # Replace "pass" statement with your code
        N,C,H,W = dout.shape
        dout_reshape = dout.permute(1,0,2,3).reshape(C,-1).t()
        dx_reshape, dgamma, dbeta = BatchNorm.backward(dout_reshape,cache)
        dx = dx_reshape.t().reshape(C,N,H,W).permute(1,0,2,3)
        ##################################################################
        #                       END OF YOUR CODE                         #
        ##################################################################
