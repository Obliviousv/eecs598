...
class Linear(object):
    ...
    def forward(x, w, b):
        ...
        N  = x.shape[0]
        out = x.reshape(N,-1).mm(w) + b.reshape(1,-1)
        ######################################################################
        #                        END OF YOUR CODE                            #
        ######################################################################
        cache = (x, w, b)
...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        d_out1, d_W2, d_b2 = Linear.backward(d_scores,cache_2)
        _, d_W1, d_b1 = Linear_ReLU.backward(d_out1,cache_1)
        d_W2 += 2*self.reg*W2
        d_W1 += 2*self.reg*W1
        grads['W2'] = d_W2
        grads['W1'] = d_W1
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        #######################################################################
        # Replace "pass" statement with your code
        dim_list = deepcopy(hidden_dims)
        dim_list.insert(0,input_dim)
        dim_list.append(num_classes)
        for k in range(self.num_layers):
            self.params['W'+str(k+1)] = torch.randn((dim_list[k],dim_list[k+1]), dtype=dtype, device=device)*weight_scale
            self.params['b'+str(k+1)] = torch.zeros(dim_list[k+1], dtype=dtype, device=device)*weight_scale
    ...
    def loss(self, X, y=None):
        ...
        k = 0
        out = X.clone()
        while (k < (self.num_layers-1)):
            out, cache = Linear_ReLU.forward(out, self.params['W'+str(k+1)], self.params['b'+str(k+1)])
            cache_list.append(cache)
            if self.use_dropout:
              out, cache_drop = Dropout.forward(out,self.dropout_param)
              cache_drop_list.append(cache_drop)
            out_list.append(out)
            k += 1
        out_final, cache_final = Linear.forward(out_list[-1], self.params['W'+str(k+1)], self.params['b'+str(k+1)])
        scores = out_final
        ...
        loss, d_scores = softmax_loss(scores,y)
        for k in range(self.num_layers):
          W = self.params['W'+str(k+1)]
          loss += self.reg*torch.sum(W*W)

        d_out, d_W, d_b = Linear.backward(d_scores,cache_final)
        grads['W'+str(self.num_layers)] = d_W + 2*self.reg*self.params['W'+str(self.num_layers)]
        grads['b'+str(self.num_layers)] = d_b
        k = self.num_layers-1
        while k>0:
            if self.use_dropout:
              d_out = Dropout.backward(d_out,cache_drop_list[k-1])
            d_out, d_W, d_b = Linear_ReLU.backward(d_out,cache_list[k-1])
            grads['W'+str(k)] = d_W + 2*self.reg*self.params['W'+str(k)]
            grads['b'+str(k)] = d_b
            k -= 1
        ###########################################################
        #                   END OF YOUR CODE                      #
...
def create_solver_instance(data_dict, dtype, device):
    model = TwoLayerNet(hidden_dim=200, reg=1e-3, dtype=dtype, device=device)
    #############################################################
