...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    x_train_ = x_train.view(num_train,1,-1)
    x_test_ = x_test.view(1,num_test,-1)
    dists = torch.sum((x_train_ - x_test_)**2, dim = 2)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists

def get_modal_number(p: torch.Tensor):
    p_table = torch.bincount(p)
    s = p_table.shape[0]
    max_v = 0
    max_id = 0
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    y_pred = torch.zeros(num_test, dtype=torch.int64).cuda()
    ##########################################################################
    ...
    for i in range(num_test):
        value, id = torch.topk(dists[:,i], k=k, largest=False)
        p = y_train[id]
        y_pred[i] = get_modal_number(p)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return y_pred
...
def knn_cross_validate(
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    x_train_folds = torch.chunk(x_train, chunks=num_folds, dim=0) # tuple
    ...
    for k in k_choices:
        for i in range(num_folds):
            x_train_split = torch.cat([x_train_folds[a] for a in range(num_folds) if a!=i], dim=0)
            y_train_split = torch.cat([y_train_folds[a] for a in range(num_folds) if a!=i], dim=0)
            x_test_split = x_train_folds[i]
            y_test_split = y_train_folds[i]
            classifier = KnnClassifier(x_train_split, y_train_split)
            k_to_accuracies[k][i] = classifier.check_accuracy(x_test_split, y_test_split, k=k)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    k_to_accuracies_mean = []
    k_list = []
    for k_v in k_to_accuracies.keys():
        average_acc = 0
        for acc in k_to_accuracies[k_v]:
            average_acc += acc
        k_to_accuracies_mean.append(average_acc)
        k_list.append(k_v)
    max_v = max(k_to_accuracies_mean)
    max_id = k_to_accuracies_mean.index(max_v)
    best_k = k_list[max_id]
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return best_k
