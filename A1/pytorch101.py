...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    right = stop//10*10 + 1
    if left < right:
        x = torch.arange(left, right, 10, dtype=torch.float64)
    else:
...
def slice_assignment_practice(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    target = torch.tensor([[0,1,2,2,2,2],[0,1,2,2,2,2],[3,4,3,4,5,5],[3,4,3,4,5,5]])
    x[:4,:6] = target
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    id1 = torch.argmin(x,dim=1)      
    id2 = torch.arange(x.shape[0])
    y = x.clone()
    y[id2,id1] = 0                                                                                             
    ##########################################################################
    #                            END OF YOUR CODE                            #
...
def batched_matrix_multiply_loop(x: Tensor, y: Tensor) -> Tensor:
    ...
    for i in range(x.shape[0]):
        list.append(x[i].mm(y[i]))
    z = torch.stack(list,dim=0)
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
    return z
